...
import react.*
import react.dom.h1
import react.router.dom.HashRouterComponent
import react.router.dom.hashRouter
import react.router.dom.route
import styled.css
import styled.styledDiv
import util.SlideHandler
...
internal class PresentationProps(
        val slides: List<Pair<SlideData, SlideHandler>>,
...
internal fun PresentationProps.transitionSet(index: SlidePosition, select: SlideData.() -> Transition.Set?) = slideInfos(index).select() ?: defaultTransitions

private val Presentation = functionalComponent<PresentationProps> { props ->
        ...
        fun RBuilder.slide(position: SlidePosition, transition: Transition?, transitionState: TransitionState?, keyPostfix: String = ""/*, overviewDelta: Int? = null*/) {
            styledDiv {
                ...
                if (slidePair != null) {
                    ...
                    child(component = Slide, props = jsObject { this.state = position.state }) {
...
interface KPresentationBuilder {
    fun slide(data: SlideData = SlideData(), handler: SlideHandler)
}

fun RBuilder.presentation(
        defaultTransition: Transition.Set = Fade,
        transitionDuration: Int = 500,
        builder: KPresentationBuilder.() -> Unit
) {
    val slides = mutableListOf<Pair<SlideData, SlideHandler>>()
    object : KPresentationBuilder {
        override fun slide(data: SlideData, handler: SlideHandler) {
            slides += data to handler
        }
    }.builder()

    if (slides.isEmpty()) {
        h1 { +"No slides provided!" }
        return
    }
    ...
    hashRouter {
        route<RouteProps>("/:slide?/:state?") {
            ...
            val modes = URLSearchParams(it.location.search).get("mode")
                    ?.split(",")
                    ?.mapNotNull { modeAsString -> try { Mode.valueOf(modeAsString.toUpperCase()) } catch (_: Throwable) { null } }
                    ?.toSet()
                    ?: emptySet()
